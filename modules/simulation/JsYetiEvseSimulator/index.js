...
function publish_oob(mod) {
  let data = {
    ...
    rcd_current:    mod.simulation_data.rcd_current,
    pp_resistor:    mod.simulation_data.pp_resistor,
  };

  if (use_pipes) {
    process.stdout.write(`${JSON.stringify(data)}\n`);
  } else {
    mod.mqtt.publish('everest_external/oob/yeti/evse', JSON.stringify(data));
  }
}

function process_oob(mod, data) {
  if (data.source === 'yeti/ev') {
    mod.ev_max_current             = data.ev_max_current;
    mod.ev_three_phases            = data.ev_three_phases;
    mod.simulation_data.cp_voltage = data.cp_voltage;
    mod.simulation_data.error_e    = data.error_e;
    mod.simulation_data.diode_fail = data.diode_fail;
    mod.simulation_enabled         = data.simulation_enabled;
  }
}
...
function subscribe_oob(mod) {
  if (use_pipes) {
    ...
    mod.mqtt.subscribe('everest_external/oob/yeti/ev', (_, chunk) => {
      process_oob(mod, JSON.parse(chunk));
    });
  }
}

async function boot({setup, info, config, mqtt}) {
  global_info = info;
