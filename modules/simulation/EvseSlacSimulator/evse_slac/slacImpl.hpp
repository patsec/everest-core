...
#ifndef EVSE_SLAC_SLAC_IMPL_HPP
#define EVSE_SLAC_SLAC_IMPL_HPP
...
public:
    slacImpl() = delete;
    slacImpl(Everest::ModuleAdapter* ev, const Everest::PtrContainer<EvseSlacSimulator>& mod, Conf& config) :
        slacImplBase(ev, "evse_slac"), mod(mod), config(config){};
...
private:
    const Everest::PtrContainer<EvseSlacSimulator>& mod;
    const Conf& config;
