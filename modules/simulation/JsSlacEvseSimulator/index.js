...
function publish_oob(mod) {
  let data = {
    source: 'slac/evse',
    state:  state_to_string(state_evse),
  };

  if (use_pipes) {
    process.stdout.write(`${JSON.stringify(data)}\n`);
  } else {
    mod.mqtt.publish('everest_external/oob/slac/evse', JSON.stringify(data));
  }
}

function process_oob(mod, data) {
  if (data.source === 'slac/ev') {
...
function subscribe_oob(mod) {
  if (use_pipes) {
    ...
    mod.mqtt.subscribe('everest_external/oob/slac/ev', (_, chunk) => {
